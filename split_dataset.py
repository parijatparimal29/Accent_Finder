...
def filter_df(df):
    ...
    for native in df['native_language'].unique():
        if native != 'english':
        lang = df[df['native_language'] == native]
            if flag == 0:
                df = english.append(lang)
            else:
                df = df.append(lang)'''
...
if __name__ == '__main__':
    ...
    #df = pd.read_csv('/Users/Rahul/Desktop/MSIS/NLP/Accent Finder/Audio files/bio_metadata.csv')
